...
namespace mtp
{
	...
	struct TrustedApp::Keys
	{
		...
		static ByteArray Encrypt(const ByteArray & key, const u8 * src, size_t size)
		{
			AES_KEY aesKey;
			...
			ByteArray iv(16);
			ByteArray result(size);
			AES_cbc_encrypt(src, result.data(), size, &aesKey, iv.data(), 1);
			return result;
		}
		...
		ByteArray GetDeviceRSAModulus() const
		{
			return _deviceRSAModulus;
		}
...
#define CHECK_MORE(ARRAY, SIZE) if (src - (ARRAY).data() + static_cast<size_t>(SIZE) > (ARRAY).size()) \
	throw std::runtime_error("input buffer overrun");
			...
			//HexDump("decrypted payload", payload);
	...
	void TrustedApp::Authenticate()
	{
		...
		_deviceRSAModulus = _keys->GetDeviceRSAModulus();
		if (!_deviceRSAModulus.empty()) {
			debug("extracted device RSA public key (", _deviceRSAModulus.size(), " bytes)");
		} else {
			debug("warning: could not extract device RSA key from decrypted payload");
		}
	...
	ByteArray TrustedApp::EncryptWiFiPassword(const std::string &password)
	{
		...
		// Convert device modulus (little-endian binary) to BIGNUM
		...
		if (!BN_bin2bn(reversed_modulus.data(), reversed_modulus.size(), n) || !BN_set_word(e, EXPONENT))
		{
			...
			BN_free(e);
			RSA_free(rsa);
			throw std::runtime_error("Failed to set device RSA parameters");
		}
		...
		// Encode password as UTF-16LE (as Zune expects)
		ByteArray utf16le_password;
		utf16le_password.reserve(password.length() * 2);

		for (char c : password) {
			utf16le_password.push_back(static_cast<u8>(c));
			utf16le_password.push_back(0);  // UTF-16LE encoding for ASCII
		}

		// Encrypt with RSA PKCS#1 v1.5 padding
		ByteArray encrypted(RSA_size(rsa));
		int result = RSA_public_encrypt(
			utf16le_password.size(),    // Length of UTF-16LE encoded password
			utf16le_password.data(),    // Plaintext data
		...
		debug("encrypted WiFi password (", encrypted.size(), " bytes) with device-specific RSA key");
